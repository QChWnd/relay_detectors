# only for gost **default** relay+tls/ws(s)/mws(s)
import requests
import argparse
import warnings
import os
...
if args.tls:
    if jarm(args.host, int(args.port)) == '3fd21b20d3fd3fd21c43d21b21b43d1ec49a4b64df0a9e9f328abd60285841':
        ...
        print('jarm mismatched')
    cert = OpenSSL.crypto.load_certificate(OpenSSL.crypto.FILETYPE_PEM, ssl.get_server_certificate((args.host, args.port)))
    if cert.get_version() == 2 and cert.get_signature_algorithm() == b'sha256WithRSAEncryption' and cert.get_pubkey().bits() == 2048 and datetime.datetime.strptime(cert.get_notAfter().decode("ascii"), '%Y%m%d%H%M%SZ').timestamp() - datetime.datetime.strptime(cert.get_notBefore().decode("ascii"), '%Y%m%d%H%M%SZ').timestamp() == 315360000:
    ...
    r = requests.get(addr + path, verify=False)
    if r.status_code == 400 and r.content == b'Bad Request\n':
        print('route /[path] matched')
    else:
        print('route /[path] mismatched')
else:
